...
export async function generateMetadata({ params }: Props) {
  ...
  const { data } = await supabase
    ...
    .select('ticker, company_name, period, verdict')
    .eq('id', params.id)
    .single();

  if (!data) return { title: 'Analysis — EarningsEdge' };
  return {
    title: `${data.ticker} ${data.period} Earnings — EarningsEdge`,
    description: `${data.company_name} ${data.period} earnings analysis`,
  };
}
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-500 mt-1 text-sm">
            {a.period} Earnings Report
            {a.report_date ? ` · Reported ${a.report_date}` : ''}
          </p>
        </div>
